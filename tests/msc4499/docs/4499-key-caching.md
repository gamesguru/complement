...
observation for that binding, as described under [Historical event
verification](#historical-event-verification).
...
inbound request, defeating the backoff entirely. Implementations MUST coalesce
concurrent outgoing key fetch requests for the same remote domain into a single
active HTTP request to prevent network saturation, except where a stronger
local trust source already satisfies the request without any outbound fetch. If
that fetch succeeds and
...
memory-only storage is insufficient for First Seen Wins. Freshness metadata
such as `valid_until_ts`, retry state, and other cache-management fields may be
...
of the 3,000-key retention ceiling described under [Storage
considerations](#storage-considerations), which governs only the bindings a
notary actively serves. However, the forensic index MUST still be bounded:
implementations MUST either retain only bounded digest metadata (origin, key
ID, digest, timestamps, and reason for rejection), or enforce explicit
per-origin and global limits if they retain full rejected key bodies.
...
HTTP 200 for the enclosing `/_matrix/key/v2/query` response, omit that
response from the `server_keys` array, and MAY continue serving other valid
...
`valid_until_ts` it is also less than the `valid_until_ts` asserted when the
key was active), and (2) the event signature cryptographically validates. Room
...
same remote server, implementations MUST coalesce them into a single active
fetch attempt for that server: at any given time, there MUST be at most one
...
later pruned, a future body reusing that evicted key ID is still checked
against the original digest and rejected if it conflicts, rather than being
treated as first seen again. Because both the
